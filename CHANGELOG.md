...
- **装配最小刚体变换**：`make_gear` 支持显式 `origin` / `axis` / `reference_direction` 并在生成时应用变换；装配实例支持平移、轴角与 3×3 正交旋转矩阵，STEP 导出与干涉检查使用变换后的几何。
- **对话决策体验**：`ask_user` 与参数确认卡片统一为决策卡视觉，显示回答进度与必答缺失原因；数字、布尔、JSON 参数分别使用专用输入并阻止非法值提交，支持恢复原值。
- **模型参数输入稳健性**：温度、Token、超时、重试输入保留草稿，越界 / 非整数 / 非法数字给出内联错误，失焦恢复上一个有效值；档案切换与项目加载正确同步外部值。
- **启动更新检查**：新增 `GET /api/update/check` 与非打扰式更新提示，只链接 GitHub Release，不自动下载、解压、覆盖或重启；支持 6 小时缓存、beta/stable 通道、仓库 / 超时 / 开关环境变量，网络失败静默降级。
- **质量门禁**：frontend 158、backend 502、kernel 503 全部通过；`compileall`、benchmark dry-run 与前端 production build 通过。

## v0.22.2-beta - 模型控制台与真实沉孔 BRep 几何 (2026-09-23)

- **模型控制台体验重构**：
  - 生效模型预览明确区分 project / env / none 来源，逐字段显示 Key、Base URL、模型与协议来自哪里；
  - 支持把当前视觉或规划模型保存到本机 `.env`，同时保存为该供应商的可切换档案；
  - 每个角色可保存多个供应商档案，点击“已存”供应商一键切换；
  - 切换档案会清空该角色项目覆盖，下一条消息从 `.env` 生效，正在运行的任务保持启动时快照；
  - `.env` 写入采用合并式原子替换，保留无关变量与注释；按角色保存不会误清空另一角色。
- **密钥安全语义**：完整 API Key 不回传前端；项目密钥掩码由服务端解析并复制到 `.env`，接口只返回末 4 位；显式空值表示清空项目覆盖并走 `.env` 兜底。
- **真实沉孔几何**：MechKernel `hole` 的 counterbore 改为两段同轴圆柱切除，保留真实肩台平面；小径、大径、肩台深度与轴线可通过现有 BRep topology query / measurement 验收；盲孔深度来自几何证据而不是参数伪装。
- **阶梯孔内核支持**：新增 `stepped` hole 操作，支持两级及以上逐段变小孔段并校验非法参数；该能力当前在内核 op 层，尚未进入高层特征面板。
- **稳健性**：过期拓扑 ID 返回结构化 `topology_not_found`，不把无关几何伪装成测量成功；退化沉孔、非有限参数、切穿肩台等输入明确失败。
- **质量门禁**：frontend 143、backend 489、kernel 488 全部通过；`compileall`、benchmark dry-run 与前端 production build 通过。
