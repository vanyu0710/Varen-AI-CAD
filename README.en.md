...
The published Windows package checked on GitHub is **v0.22.3-beta** (about 180 MiB). The source [VERSION](VERSION) is `0.22.3-beta`. See [project status](docs/PROJECT_STATUS.md).

1. Open the [v0.22.3-beta release](https://github.com/vanyu0710/Varen-AI-CAD/releases/tag/v0.22.3-beta). Download `VarenCAD-win64-0.22.3-beta.zip` and its `.sha256` attachment under **Assets**, not the source-code archives. Run `Get-FileHash .\VarenCAD-win64-0.22.3-beta.zip -Algorithm SHA256` and compare the result with the checksum file.
